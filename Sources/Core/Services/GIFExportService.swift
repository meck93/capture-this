...
public struct GIFExportPolicy: Equatable, Sendable {
  ...
  public init(
    ...
    warningFileSize: Int64
  ) {
    self.maxDuration = maxDuration
    self.targetWidth = targetWidth
    self.targetFramesPerSecond = targetFramesPerSecond
    self.warningFileSize = warningFileSize
  }
}

public struct GIFExportEstimate: Equatable, Sendable {
...
public final class GIFExportService {
  ...
  public func estimate(for recording: Recording) async throws -> GIFExportEstimate {
    ...
    let estimatedFileSize = estimateFileSize(frameCount: frameCount, scaledSize: size)

    return GIFExportEstimate(
      duration: duration,
      frameCount: frameCount,
      estimatedFileSize: estimatedFileSize,
      exceedsWarningThreshold: estimatedFileSize > policy.warningFileSize
    )
  }
  ...
  public func estimateFileSize(frameCount: Int, scaledSize: CGSize) -> Int64 {
    let pixels = max(1, Int(scaledSize.width.rounded()) * Int(scaledSize.height.rounded()))
    let bytesPerFrame = max(32000, pixels / 3)
